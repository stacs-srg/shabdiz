package uk.ac.standrews.cs.artisan.worker.servers;
...
import uk.ac.standrews.cs.artisan.worker.UnreachableCoordinatorException;
import uk.ac.standrews.cs.artisan.worker.WorkerNodeFactory;
...
public class WorkerNodeServer {
     ...
     * <dd>Specifies the local address and port at which the Artisan Worker service should be made available.</dd>
     *
     * <dt>-khost:port (optional)</dt>
     * <dd>Specifies the address and port of an existing Artisan coordinator, by which the new node is coordinated.</dd>
     *
    ...
    public static void main(final String[] args) throws RPCException, UndefinedDiagnosticLevelException, IOException, AlreadyBoundException, RegistryUnavailableException, InterruptedException, TimeoutException, UnreachableCoordinatorException {
        ...
        try {
            server.createNode();
            Diagnostic.trace("Started Artisan worker node at " + server.local_address);
        }
        catch (final IOException e) {
            Diagnostic.trace("Couldn't start Artisan worker node at " + server.local_address + " : " + e.getMessage());
        }
    }
