...
/**
 * A factory for creating McJobNode objects.
 * 
...
public class WorkerNodeFactory extends P2PNodeFactory { // XXX discuss whether p2pnodefactory is a good name for that class since its factory for node, not necessarily a p2p node
    ...
    /**
     * Creates a new McJob node running in the current JVM at a given local network address on a given port, establishing a new one-node ring.
     *
    ...
    /**
     * Binds to an existing remote McJob node running at a given network address, checking for liveness.
     *
    ...
    /**
     * Binds to an existing remote McJob node running at a given network address, checking for liveness, retrying on any error until the timeout interval has elapsed.
     *
     * @param node_address the address of the existing node
     * @return a remote reference to the node
     *
