...
import uk.ac.standrews.cs.mcjob.worker.UnreachableCoordinatorException;
...
public class WorkerNodeServer {
     ...
     * <dd>Specifies the local address and port at which the McJob Worker service should be made available.</dd>
     *
     * <dt>-khost:port (optional)</dt>
     * <dd>Specifies the address and port of an existing McJob Worker node, via which the new node should join the ring.</dd>
     *
    ...
    public static void main(final String[] args) throws RPCException, UndefinedDiagnosticLevelException, IOException, AlreadyBoundException, RegistryUnavailableException, InterruptedException, TimeoutException, UnreachableCoordinatorException {
        ...
        try {
            server.createNode();
            Diagnostic.trace("Started McJob Worker node at " + server.local_address);
        }
        catch (final IOException e) {
            Diagnostic.trace("Couldn't start McJob Worker node at " + server.local_address + " : " + e.getMessage());
        }
    }
